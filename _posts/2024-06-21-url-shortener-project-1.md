...
**짧아진 링크를 살펴보면 도메인인 `https://bit.ly/`뒤에 `3VzBS9q`라는 문자 조합이 붙어 있는 것을 확인할 수 있다. 이 문자 조합을 숏키(shortkey) 또는 숏코드(shortcode)라고 부른다. URL 단축의 핵심은 원본 URL을 유일한 숏코드로 매핑하는 로직을 구현하는 것이다.**
...
## 3. URL 단축 방법 - 1(Hash Function + Base62)
...
* 기존 방법과 다르게 해시값을 자르지 않는다. 해시값을 Base62 인코딩해서 나온 결과를 앞 7자리를 추출한다.
* 이렇게 인코딩해서 앞 7자리 추출한 값을 원본 URL의 식별자로 사용하고, 숏코드로도 사용한다
* 이 방법은 다음의 장점을 가진다
...
URL 단축을 구현하는 두 번째 방법은 **원본 URL에 대한 식별자로 시퀀스(Sequence)를 사용하는 방법**이다. 쉽게 말해서 이전 처럼 원본 URL에 해시 함수를 사용해서 얻은 해시값을 사용하는 것이 아니라, **1씩 증가하는 `id`를 식별자로 사용**하는 것이다.
...
1. 원본 URL에 해시 함수를 이용해서 나온 해시값을 Base62 인코딩해서 숏코드를 생성하는 방식
   * 원본 URL을 조회하기 위한 식별자를 해시값으로 설정하는 경우
   * 식별자를 숏코드로 설정하는 경우
