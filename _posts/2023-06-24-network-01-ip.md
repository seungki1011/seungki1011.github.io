...
categories: [CS, 네트워크]
tags: [network, ip, subnetting, vlsm, flsm]
math: true
mermaid: true
---
...
뒤의 [IP 주소 체계]()에서 다루겠지만, 우리는 현재 IP 주소로 IPv4 프로토콜을 사용하고 있다.
...
클래스 방식을 간략히 설명하자면 **서브넷 마스크에 따라 클래스 별로 아이피의 범위를 분류하는 방식**이다.
...
만들어진 서브넷을 그림을 확인하면 다음과 같은 모습이다. `2`비트를 빌렸기 때문에 각 서브넷의 길이는 `26`이 된다.
...
* 가변 길이 서브넷 마스크(VLSM)이라고 불리는 이유는 각 서브넷의 `네트워크 ID`의 서브넷 마스크가 `28`로 동일하지 않고 `28`, `29`로 가변적이기 때문이다
