...
가장 대표적인 문제가 **다중 생산자와 다중 소비자가 동시에 버퍼에 접근해서 경쟁 상태(race condition)가 발생하는 경우**이다. 이로 인해서 데이터의 일관성이 깨질 수 있다. 또 다른 문제는 **버퍼 오버플로우(overflow)와 버퍼 언더플로우(underflow)**이다. 이 문제의 경우 **버퍼가 가득 찬 상태에서 생산자가 아이템을 추가**하거나, **버퍼가 비어있는 상태에서 소비자가 아이템을 소비하려고 시도할 때 발생**한다.
