...
>
> 로그는 `logstash-logback-encoder`를 통해 JSON 형태로 파싱하고 있다. 만약 로깅에 익숙하지 않다면 로깅 대신 `System.out`을 사용하면 된다.
{: .prompt-warning }
...
한번 간단하게 사용해보자. 다음의 `HelloRunnable` 인터페이스를 만들자.
...
```
{
  ...
  "@version" : "1",
  "message" : "main() start",
  "logger_name" : "de.thread.ManyThreadMain",
  "thread_name" : "main",
  "level" : "INFO",
  "level_value" : 20000
}
...
}
{
  ...
  "logger_name" : "de.thread.HelloRunnable",
...
자바에서는 모든 user 스레드가 종료되기 전까지 JVM은 종료되지 않는다. 그렇기 때문에 `main` 스레드가 종료되어도, `Thread-0`가 종료되기 전까지 JVM은 살아있다.
...
![Inheritance](../post_images/2024-02-03-java-25-thread/threadstack.png)_https://medium.com/@Ravjan/java-threads-part-1-basics-5a131943979d_
...
![Inheritance](../post_images/2024-02-03-java-25-thread/single.png)_싱글 스레드에서의 I/O 블락킹_



<br>

![Inheritance](../post_images/2024-02-03-java-25-thread/multi.png)_멀티 스레드에서의 I/O 블락킹_
...
<br>
 ...
 ![thread](../post_images/2024-02-03-java-25-thread/local1.png)_동시성 문제_
...
<br>
 ...
 ![thread](../post_images/2024-02-03-java-25-thread/local2.png)
...
<br>
 ...
 ![thread](../post_images/2024-02-03-java-25-thread/local4.png)
